...
When a backend adapter still returns a composer-proven `pending` (herdr emits that final verdict only from a busy pre-Enter baseline whose composer read proved the typed text), the shared dispatch layer (`fm_backend_send_text_submit` in `bin/fm-backend.sh`) runs a hoisted read-back on herdr backends (where native agent-state busy detection is available): a provably busy pane whose bounded capture holds the typed text (condensed, matched on a distinctive middle window so a stale digest sharing the constant envelope head and scaffold tail never counts) upgrades the verdict to the proof-carrying `queued-busy`; `pending-unproven` (herdr's idle-baseline Enter exhaustion or an ambiguous composer) and `unknown` verdicts are never rescued and stay delivery failures.
The daemon and `fm-send` accept `queued-busy` as delivery - the daemon logs `inject queued` and clears the escalation buffer - so herdr's OpenCode busy-queue case is rescued once at the dispatch layer instead of per caller.
...
[`AGENTS.md` section 7](../AGENTS.md#7-task-lifecycle) owns task delivery and merge authority; the headers of `bin/fm-brief.sh`, `bin/fm-spawn.sh`, and `bin/fm-promote.sh` own their required inputs.
