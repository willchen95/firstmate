...
When a backend adapter still returns an inconclusive `pending` or `unknown`, the shared dispatch layer (`fm_backend_send_text_submit` in `bin/fm-backend.sh`) runs a hoisted read-back: a provably busy pane whose bounded capture holds the typed text (condensed, matched on a distinctive middle window so a stale digest sharing the constant envelope head and scaffold tail never counts) upgrades the verdict to the proof-carrying `queued-busy`.
