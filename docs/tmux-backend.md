...
The dispatch-layer read-back in `bin/fm-backend.sh` upgrades a `pending` verdict to the proof-carrying `queued-busy` only on herdr backends (where native agent-state busy detection is available) and only when the typed text is visible in a bounded capture; tmux has no native busy state, so verdicts this adapter leaves inconclusive pass through unchanged, and [architecture](architecture.md) owns that dispatch-layer contract.
