...
- **Explicit project modes** - each project ships via `no-mistakes`, `direct-PR`, or `local-only`, with an optional `+yolo` merge-authority flag ([authority policy](AGENTS.md#selected-delivery-path-and-approval-authority)).
